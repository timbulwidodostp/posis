...
/*
	purpose:
		...
		screening using -lasso-, -stepbic-, or -steptest- for the
		linear, logit, and Poisson models.
	
	syntax:
		posis depvar dvars [if] [in], 		///
			controls(varlist)		///
...
program ParseSyntax, rclass
	...
	gettoken selcmd tmp : method, parse(",")
	...
	if (`"`selcmd'"' != "lasso" 		///
		...
		di as err "option {bf:method()} allows only one " 	///
			"of {bf:(lasso,cv)}, {bf:(lasso,plugin)}, "	///
			"{bf:(lasso,adaptive)}, {bf:(lasso,bic)}, "	///
			"{bf:steptest}, or {bf:stepbic}"
		exit 198
	}
...
program ComputeGLM, eclass
	...
	di_log, var(`yvar') method(`method')
					...
					// isis y on d and x 
	qui isis `yvar' `dvars' `xvars', model(`model') 	///
		method(`method') `maxiter'
	local controls_sel `e(allvars_sel)'
	...
	foreach var of local dvars {
		...
		qui isis `dvar_tmp' `xvars' [iw=`wvar'], model(linear) ///
			method(`method') `maxiter'
		local controls_sel_k `controls_sel_k' || `e(allvars_sel)'
...
program Head
	...
	local model as txt "Model: " as res "`e(model)'"
						...
						// method
	local method as txt "Method: " as txt `"`e(method)'"'
...
program PostResult, eclass
	...
	local mymethod = ustrregexra(`"`method'"', ",", "")
	eret local title "Partialing-out ISIS"
	eret local method "`mymethod'"
...
program ComputeLinear, eclass
	...
	di
	di_log, var(`yvar') method(`method')
	qui isis `yvar' `xvars', method(`method') model(linear) `maxiter'

					// housekeeping
	local controls_sel `e(allvars_sel)'
	local controls_sel_k `e(allvars_sel)'
	local depvar_k `yvar'
					// get residuals
	tempvar rho
	...
	foreach var of local dvars {
		...
		qui isis `dvar_tmp' `xvars', model(linear)	///
			method(`method') `maxiter'
...
program di_log
	...
	local mymethod = ustrregexra(`"`method'"', ",", "")
	di as txt "select controls for {bf:`var'} using ISIS {bf:`mymethod'}"
end
...
program ComputeChi2, eclass
	...
	eret hidden scalar chi2 = `chi2'
	eret hidden scalar p = `p'
	eret hidden scalar df = `df'
	eret hidden local chi2type "Wald"
