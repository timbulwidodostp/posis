...
program ComputeGLM, eclass
	...
	di_log, var(`yvar') method(`method')
					...
					// lasso cv
	qui lasso `estcmd' `yvar' `dvars' `xvars', sel(cv)
	local xvars_sel `e(allvars_sel)'
					// swcmd y on d and x 
	qui `swcmd' : `estcmd' `yvar' `dvars' `xvars_sel'	
	...
	foreach var of local dvars {
		...
		di_log, var(`var') method(`method')
					...
					// lasso cv
		qui lasso linear `dvar_tmp' `xvars' [iw=`wvar'], sel(cv)
		local xvars_sel `e(allvars_sel)'

					// stepwise
		qui `swcmd': regress `dvar_tmp' `xvars_sel' [iw=`wvar']
...
program PostResult, eclass
	...
	eret local title "Partialing-out hybrid stepwise `method'"
...
program ComputeLinear, eclass
	...
	di_log, var(`yvar') method(`method')
					...
					// lasso cv
	qui lasso linear `yvar' `xvars', sel(cv)
	local xvars_sel `e(allvars_sel)'

					// stepwise
	qui `swcmd' : regress `yvar' `xvars_sel'
	...
	foreach var of local dvars {
		...
		di_log, var(`var') method(`method')
					...
					// lasso cv
		qui lasso linear `dvar_tmp' `xvars', sel(cv)	
		local xvars_sel `e(allvars_sel)'

					// stepwise
		qui `swcmd': regress `dvar_tmp' `xvars_sel'
...
program di_log
	...
	di as txt "select controls for {bf:`var'} using "	///
		"hybrid stepwise {bf:`method'}"
end
